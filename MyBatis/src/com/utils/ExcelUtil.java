...
public class ExcelUtil {
    ...
    public static ArrayList<ArrayList<Import_table_format>> readExcel(File file) throws Exception{  
        if(file == null){  
            return null;  
        }  
...
//        }
//
//        double E = 0,content = 0,consume = 0,activity = 0,transI=0.001,SVi = 0;
//        String unit = "";
//        int foodCountSum = 0;
//		List<SaveE> saveElistNew = new ArrayList<SaveE>();
//		List<SVTModel> SVTModelList = new ArrayList<SVTModel>();
//		List<SVTRAModel> SVTRAModelList = new ArrayList<SVTRAModel>();
//		double consumeType = 0,contentType = 0,SVT = 0,consumeSum = 0,SVTConsume = 0,SVTRA = 0;
//		int foodCountType = 0;
//		session = Constants.getSqlSession();
//		saveEDao = session.getMapper(SaveEDao.class);
//		List<Radio> radioList = saveEDao.queryRadioList();
...
//			foodCountSum++;
//		}
//		for(SaveE aOldSaveE:saveElist){
//			E = aOldSaveE.getE();
//			age = aOldSaveE.getAge();
//			radio = aOldSaveE.getRadio();
//			consumeSum = 0;
//			SVTConsume = 0;
//				for (Map.Entry<String,  List<String>> entry : foodNameMap.entrySet()) {
//					consumeType = 0;
...
//					foodCountType = entry.getValue().size();
//					for(Radio aRadio:radioList){
//						if(aRadio.getRadio().equals(aOldSaveE.getRadio())){
//						activity = aRadio.getActivity();
//						unit = aRadio.getUnit();
//						continue;
//						}
//					}
//					SVT = 0;
//					for(String foodName:entry.getValue()){		
//						for(FoodConsume aConsume:foodConsumeList){
//							if((changeAgeToTwoTable(aConsume.getAge()).equals(age))&&(aConsume.getFoodName().equals(foodName))){
//								consume = aConsume.getFoodConsume();
////								age = aConsume.getAge();
//								break;
//							}
...
//							}
//						}
//						if(age.equals("0-1岁")||age.equals("1-2岁"))
//							continue;
//						if(unit.equals("mg/kg")){
//							SVi = E*content*consume*activity*transI;
//						}
...
//						aSaveE.setE(E);
//						aSaveE.setFoodName(foodName);
//						aSaveE.setFoodType(entry.getKey());
//						aSaveE.setActivity(activity);
//						aSaveE.setUnit(unit);
//						aSaveE.setFoodConsume(consume);
//						aSaveE.setContent(content);
...
//					aSVTModel.setAge(age);
//					aSVTModel.setE(E);
//					aSVTModel.setFoodType(entry.getKey());
//					aSVTModel.setActivity(activity);
//					aSVTModel.setUnit(unit);
//					aSVTModel.setFoodConsume(consumeType);
//					aSVTModel.setFoodCount(foodCountType);
//					aSVTModel.setContaver(contentType/foodCountType);
//					aSVTModel.setTransI(transI);
//					aSVTModel.setActivity(activity);
//					aSVTModel.setSVT(SVT);
//					SVTModelList.add(aSVTModel);
//					consumeSum += consumeType; 
//					SVTConsume += (SVT*consumeType); 
//					
//					
//		        }
//			
//			if(age.equals("0-1岁")||age.equals("1-2岁"))
//				continue;
//			SVTRA = SVTConsume/consumeSum; 
//			SVTRAModel aSVTRAModel = new SVTRAModel();			
//			aSVTRAModel.setRadio(radio);
//			aSVTRAModel.setAge(age);
//			aSVTRAModel.setFoodConsumeSum(consumeSum);
//			aSVTRAModel.setSVTRA(SVTRA);
//			SVTRAModelList.add(aSVTRAModel);
//		}
//
//		//saveElistNew存了20898条SaveE记录，存了rae表的内容含SVi end
//		//SVTModelList存了所有的SVT内容end
//		//SVTRAModelList 存了图二表头的radio/age 和副坐标SVTRA的内容 end
//		//SVTRA END
//		
//		//第一类图start
//		double contentSum = 0,contaver = 0;
...
//		//批量插入数据
//		saveEDao.insertRAETable(saveElistNew);
//		saveEDao.insertSVTTable(SVTModelList);//图二纵坐标
//		saveEDao.insertSVTRATable(SVTRAModelList);//图二副坐标与标题		
//		saveEDao.clearLKJTABLE();
//		saveEDao.insertLKJTABLE(graphOneModelList);//图一纵坐标加描点
//		saveEDao.clearRA();
